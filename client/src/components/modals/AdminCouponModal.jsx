            ...
            <div className="modal-content" style={{ width: 400 }}>
                ...
                <form onSubmit={handleSubmit}>
                    <div className="admin-form-group">
                        ...
                        <div className="admin-flex-row">
                            <input 
                                ...
                                value={formData.code} 
                                onChange={e => { setFormData({ ...formData, code: e.target.value.toUpperCase() }); toast.error(''); }} 
                                placeholder="VD: TET" disabled={isEditMode} 
                                style={{ background: isEditMode ? '#eee' : '#fff' }}
                            />
                            {!isEditMode && (
                                <button type="button" onClick={generateRandomCode} className="btn-random-code" title="Tạo ngẫu nhiên">🎲 Random</button>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="admin-form-group">
                        ...
                        <input className="admin-form-input" type="date" min={today} value={formData.expiry_date} onChange={e => { setFormData({ ...formData, expiry_date: e.target.value }); toast.error(''); }} required />
                    </div>
