...
const RightSidebar = ({ user, unreadCount, notifications, showNotifDropdown, handleToggleNotifications, handleLogout, onOpenModal }) => {
    ...
    useEffect(() => {
        axiosClient.get('/users/top-chefs').then(res => setTopChefs(res.data)).catch(err => console.log("Lỗi lấy Top Chef:", err));
        const fetchTrending = async () => {
            try { setTrending((await axiosClient.get('/recipes/trending')).data); } 
    ...
    return (
        <aside className={`sidebar-right-panel ${isPremiumModalOpen ? 'z-max' : ''}`}>
            <div className="user-panel-header">
                ...
                <div className="rs-notif-wrapper">
                    <span 
