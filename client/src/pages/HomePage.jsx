...
const HomePage = () => {
    ...
    const handleSubmitRecipe = async () => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                setUploadData({ name: '', description: '', calories: '', time: '', image: null, ingredients: [], steps: [] });
   ...
   useEffect(() => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // (Thêm logic lọc block user của bạn ở đây nếu có)
    ...
    const visibleRecipes = recipes.filter(r => !blockedUserIds.includes(r.author_id) && !blockedUserIds.includes(r.user_id));
    const visibleCommunity = communityPosts.filter(p => !blockedUserIds.includes(p.user_id) && !blockedUserIds.includes(p.author_id));
            ...
            <main className="main-content">
                ...
                {activeTab === 'community' && (
                    <CommunityView 
                        ...
                        commentText={commentText} setCommentText={setCommentText} replyingTo={replyingTo} setReplyingTo={setReplyingTo} handleViewProfile={handleViewProfile}
                    />
                )}
