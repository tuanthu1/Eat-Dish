                ...
                <FilterModal 
                    ...
                    onApply={async (newFilters) => {
                        ...
                        try {
                            ...
                            const filterParams = {};
                            if (maxCal) filterParams.maxCal = maxCal;
                            if (maxTime) filterParams.maxTime = maxTime;
                            if (ingredient) filterParams.ing = ingredient;
                            const res = await axiosClient.get('/recipes/filter', { params: filterParams });
                            setRecipes(res.data);
                            if (res.data.length === 0) toast.error("Không tìm thấy món phù hợp.");
                            else toast.success(`Tìm thấy ${res.data.length} món phù hợp!`);
                        } catch (err) {
