...
const AdminPage = () => {
    ...
    const handleDeletePackageClick = (pkg) => { setPkgToDelete(pkg); setIsConfirmDeleteOpen(true); };
    ...
    const confirmDeleteRecipe = async() => {
        ...
        try { await axiosClient.delete(`/admin/recipes/${recipeToDelete.id}`); setSuccessMsg(`Đã xóa: ${recipeToDelete.name}`); loadAllRecipes(); } 
        catch(e) { setError("Lỗi xóa công thức."); }
    }
    const confirmDeletePost = async () => {
        if (!postToDelete) return;
        try { await axiosClient.delete(`/admin/community/${postToDelete.id}`); setSuccessMsg("Đã xóa bài viết."); setIsDeletePostModalOpen(false); loadCommunityPosts(); } 
        catch (e) { setError("Lỗi xóa bài viết."); }
    };
    const confirmApprovePost = async () => {
        if (!postToApprove) return;
        try { await axiosClient.put(`/admin/community/${postToApprove.id}/approve`); setSuccessMsg("Đã duyệt."); setIsApprovePostModalOpen(false); loadCommunityPosts(); } 
        catch (e) { setError("Lỗi duyệt bài."); }
    };
    const confirmDeleteUser = async () => {
        if (!userToDelete) return;
        try { await axiosClient.delete(`/users/${userToDelete.id}`); setSuccessMsg(`Đã xóa: ${userToDelete.fullname}`); loadUsers(); } 
        catch (e) { setError("Lỗi xóa tài khoản."); }
    };
    const confirmToggleVerify = async () => {
        ...
        try {
            ...
            setSuccessMsg(newStatus === 1 ? "Đã kích hoạt! ✅" : "Đã khóa! 🔒");
        } catch (err) { setError("Lỗi cập nhật trạng thái!"); }
    };
    const confirmResetPass = async () => {
        if (!userToReset) return;
        try { await axiosClient.put(`/admin/reset/${userToReset.id}`, { password: "123456" }); setSuccessMsg(`Reset mật khẩu thành: 123456`); } 
        catch (e) { setError("Lỗi reset mật khẩu."); }
    };
    const confirmDeleteFeedback = async () => {
        if (!feedbackToDelete) return;
        try { await axiosClient.delete(`/admin/feedbacks/${feedbackToDelete.id}`); setSuccessMsg("Đã xóa góp ý."); loadFeedBacks(); } 
        catch (e) { setError("Lỗi xóa góp ý."); }
    };
    const confirmTogglePremium = async () => {
        ...
        try {
            ...
            setSuccessMsg(newStatus === 1 ? "Đã cấp VIP! 👑" : "Đã hủy VIP!");
        } catch (err) { setError("Lỗi cập nhật VIP!"); }
    };
    ...
    const confirmAddPackage = async () => {
        try { await axiosClient.post('/admin/packages', newPackage); setSuccessMsg("Đã thêm gói!"); setIsPackageModalOpen(false); loadPackages(); setNewPackage({ name: '', price: '', duration_days: 30, description: '' }); } 
        catch (err) { setError("Lỗi thêm gói"); }
    };
    const confirmDeletePackage = async () => {
        if (!pkgToDelete) return;
        try { await axiosClient.delete(`/admin/packages/${pkgToDelete.id}`); setSuccessMsg("Đã xóa gói"); loadPackages(); } 
    ...
    const openRecipeDetails = async (recipeOrId) => {
        ...
        catch (e) { setError('Lỗi tải chi tiết.'); } finally { setIsLoading(false); }
    };
    const handleSavePackage = async (formData) => {
        try {
            if (isEditMode && currentPkg) { await axiosClient.put(`/admin/packages/${currentPkg.id}`, formData); setSuccessMsg("Đã cập nhật!"); } 
            else { await axiosClient.post('/admin/packages', formData); setSuccessMsg("Đã thêm gói!"); }
            setIsPackageModalOpen(false); loadPackages();
        } catch (err) { setError("Lỗi lưu gói cước!"); }
    };
    const handleSaveCoupon = async (formData) => {
        try {
            if (isCouponEditMode && currentCoupon) { await axiosClient.put(`/admin/coupons/${currentCoupon.id}`, formData); setSuccessMsg("Đã cập nhật!"); } 
            else { await axiosClient.post('/admin/coupons', formData); setSuccessMsg("Đã tạo mã!"); }
            setIsCouponModalOpen(false); loadCoupons();
        } catch (e) { setError(e.response?.data?.message || "Lỗi tạo mã"); }
    };
    const handleDeleteCoupon = async () => {
        if (!couponToDelete) return;
        try { await axiosClient.delete(`/admin/coupons/${couponToDelete.id}`); setSuccessMsg("Đã xóa mã"); loadCoupons(); } 
    ...
    const handleToggleCouponStatus = async (coupon) => {
        ...
        } catch (e) { setError("Lỗi trạng thái!"); }
    };
    ...
    const handleExportExcel = (data, fileName) => {
        if (!data || data.length === 0) { setError("Không có dữ liệu để xuất!"); return; }
        const worksheet = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(workbook, worksheet, "Sheet1");
        XLSX.writeFile(workbook, `${fileName}.xlsx`);
        setSuccessMsg(`Đã xuất ${fileName}.xlsx!`);
    };
    ...
    return (
        <div className='admin-container'>
            ...
            <div className='admin-sidebar'>
                ...
                <div className={`admin-menu-item ${activeTab === 'dashboard' ? 'active' : ''}`} onClick={() => setActiveTab('dashboard')}><span className="menu-icon">📊</span><span>Tổng Quan</span></div>
                <div className={`admin-menu-item ${activeTab === 'users' ? 'active' : ''}`} onClick={() => setActiveTab('users')}><span className="menu-icon">👥</span><span>Người Dùng</span></div>
                <div className={`admin-menu-item ${activeTab === 'recipes' ? 'active' : ''}`} onClick={() => setActiveTab('recipes')}><span className="menu-icon">🍲</span><span>Món Ăn</span></div>
                <div className={`admin-menu-item ${activeTab === 'billing' ? 'active' : ''}`} onClick={() => setActiveTab('billing')}><span className="menu-icon">💰</span><span>Doanh Thu</span></div>
                <div className={`admin-menu-item ${activeTab === 'community' ? 'active' : ''}`} onClick={() => setActiveTab('community')}><span className="menu-icon">💬</span><span>Cộng Đồng</span></div>
                <div className={`admin-menu-item ${activeTab === 'packages' ? 'active' : ''}`} onClick={() => setActiveTab('packages')}><span className="menu-icon">💎</span><span>Gói Premium</span></div>
                <div className={`admin-menu-item ${activeTab === 'coupons' ? 'active' : ''}`} onClick={() => setActiveTab('coupons')}><span className="menu-icon">🎟️</span><span>Mã Giảm Giá</span></div>
                <div className={`admin-menu-item ${activeTab === 'feedbacks' ? 'active' : ''}`} onClick={() => setActiveTab('feedbacks')}><span className="menu-icon">📭</span><span>Góp Ý</span></div>
            ...
            <div className='admin-content'>
                ...
                {activeTab === 'users' && (
                    <div className='fadeIn'>
                        <div className="admin-header-row">
                            <h1 className='page-title'>Quản lý người dùng ({userList.length})</h1>
                            ...
                            {packages
                                ...
                                <div key={pkg.id} className="package-card">
                                    <div className="admin-action-row" style={{ position: 'absolute', top: 15, right: 15 }}>
                                        <button onClick={() => openEditPackageModal(pkg)} className="btn-icon btn-secondary" title="Sửa">✏️</button>
                                        <button onClick={() => { setPkgToDelete(pkg); setIsConfirmDeleteOpen(true); }} className="btn-icon btn-danger" title="Xóa">🗑️</button>
                                    </div>
                            ...
                            <table className="admin-table">
                                ...
                                <tbody>
                                    {couponList
                                        ...
                                        .map(c => (
                                        <tr key={c.id}>
                                            <td><span className="coupon-code-badge">{c.code}</span></td>
