...
const AdminPage = () => {
    ...
    const copyToClipboard = (text) => {
        navigator.clipboard.writeText(text)
            .then(() => setSuccessMsg('Đã sao chép!'))
    ...
    const handleSendNewsletter = async () => {
        ...
        try {
            ...
            setError(` Lỗi: ${error.response?.data?.message || 'Không thể gửi email lúc này'}`);
        } finally {
    ...
    const executeToggleMaintenance = async () => {
        try {
            ...
            setError("Lỗi khi thay đổi trạng thái bảo trì!");
            setIsMaintenanceModalOpen(false);
        }
    };
    ...
    const confirmAddPackage = async () => {
        try { await axiosClient.post('/admin/packages', { name: 'Gói mới', price: 0, duration_days: 30, description: '' }); setSuccessMsg("Đã thêm gói!"); setIsPackageModalOpen(false); loadPackages(); } 
        catch (err) { setError("Lỗi thêm gói"); }
    };
    const confirmDeletePackage = async () => {
        if (!pkgToDelete) return;
        try { await axiosClient.delete(`/admin/packages/${pkgToDelete.id}`); setSuccessMsg("Đã xóa gói"); loadPackages(); setIsConfirmDeleteOpen(false); } 
    ...
    const handleDeleteCoupon = async () => {
        ...
        try { await axiosClient.delete(`/admin/coupons/${couponToDelete.id}`); setSuccessMsg("Đã xóa mã"); loadCoupons(); setIsDeleteCouponModalOpen(false); } 
    ...
    return (
        <div className='admin-container'>
            ...
            <div className='admin-sidebar'>
                ...
                <div className={`admin-menu-item ${activeTab === 'newsletter' ? 'active' : ''}`} onClick={() => setActiveTab('newsletter')}><span className="menu-icon">📧</span><span>Gửi Email</span></div>
