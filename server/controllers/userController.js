...
exports.updateProfile = async (req, res) => {
    try {
        ...
        if (files.avatar && files.avatar[0]) {
            avatarUrl = `${process.env.BASE_URL || ''}/uploads/${files.avatar[0].filename}`;
        }

        if (files.cover_img && files.cover_img[0]) {
            coverUrl = `${process.env.BASE_URL || ''}/uploads/${files.cover_img[0].filename}`;
        }
        let updateFields = [
            "fullname = ?", 
            "bio = ?", 
        ...
        let queryParams = [fullname, bio, avatarUrl, coverUrl];
...
exports.getMutualBlockIds = async (req, res) => {
    try {
        const { userId } = req.query;
        const sql = `
            SELECT blocked_id AS block_id FROM user_blocks WHERE blocker_id = ?
            UNION
            SELECT blocker_id AS block_id FROM user_blocks WHERE blocked_id = ?
        `;
        const [list] = await db.query(sql, [userId, userId]);
        
        const blockedIds = list.map(item => Number(item.block_id));
        res.json(blockedIds);
    } catch (err) { res.status(500).json({ message: "Lỗi server" }); }
};
...
exports.getFollowers = async (req, res) => {
    try {
        const { id } = req.params; 
        const query = `
            SELECT u.id, u.fullname, u.username, u.avatar, u.bio
            FROM users u
