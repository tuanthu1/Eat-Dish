...
exports.handleWebhook = async (req, res) => {
    try {
        ...
        if (expectedSignature !== body.signature) {
            console.error("Chữ ký Webhook không khớp! Có thể là giả mạo.");
            return res.json({ success: true }); 
        }
        ...
        if (isPaid) {
            ...
            const [paymentRows] = await db.query("SELECT user_id, package_id, status FROM payments WHERE order_id = ?", [orderCode]);
            
            if (paymentRows.length > 0 && paymentRows[0].status !== 'paid') {
                const { user_id, package_id } = paymentRows[0];
                let durationDays = 0;
                if (package_id) {
                    const [pkgRows] = await db.query("SELECT duration_days FROM premium_packages WHERE id = ?", [package_id]);
                    if (pkgRows.length > 0) durationDays = pkgRows[0].duration_days;
                }
                const [userRows] = await db.query("SELECT premium_until, is_premium FROM users WHERE id = ?", [user_id]);
                const currentUser = userRows[0];
                ...
                await db.query("UPDATE payments SET status = 'paid' WHERE order_id = ?", [orderCode]);
                await db.query(
                    `UPDATE users 
                     SET is_premium = 1, 
                         premium_since = IF(is_premium = 1, premium_since, NOW()), 
                         premium_until = DATE_ADD(${startDateSql}, INTERVAL ? DAY) 
                     WHERE id = ?`,
                    [durationDays, user_id]
                );
            }
        }
        else if (data.status === "CANCELLED" || code !== "00") {
